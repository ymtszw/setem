...
const { generate, resolvePaths, resolveDependencies, getIdentifiersAndEnsureCache } = require("./index");
...
afterEach(() => {
  glob.sync(path.resolve("tmp", "*")).forEach((p) => fs.rmSync(p, { force: true }));
});
...
describe("generate()", () => {
  ...
  test("should generate from ExtensibleRecord", () => {
    expect(generate("src/fixtures/ExtensibleRecord.elm")).toBe(`-- This module is generated by \`setem\` command. DO NOT edit manually!
...
`);
  ...
  test("should generate from PatternMatch", () => {
    expect(generate("src/fixtures/PatternMatch.elm")).toBe(`-- This module is generated by \`setem\` command. DO NOT edit manually!
...
`);
  ...
  test("should generate with options", () => {
    expect(generate("src/fixtures/RecordDefAndExpr.elm", "With")).toBe(`-- This module is generated by \`setem\` command. DO NOT edit manually!
...
`);
  ...
  test("should generate with elm.json (dependencies included)", () => {
    ...
    expect(generate(sources, "RecordSetter", "s_", elmJsonFile)).toEqual(fixture);

    // Cache created
    const cacheDir = fs.readdirSync("src/fixtures/elm-spa-example/elm-stuff/setem/cache");
    expect(cacheDir.length).toBeGreaterThanOrEqual(1);
    const cacheFile = path.resolve("src/fixtures/elm-spa-example/elm-stuff/setem/cache", cacheDir[0]);
...
describe("resolvePaths()", () => {
  ...
  test("should respect explicit file paths", () => {
    expect(resolvePaths(["src/fixtures/RecordDefAndExpr.elm"])).toEqual([`${CWD}/src/fixtures/RecordDefAndExpr.elm`]);

    expect(resolvePaths(["src/fixtures/RecordDefAndExpr.elm", "src/fixtures/PatternMatch.elm"])).toEqual([
...
describe("resolveDependencies()", () => {
  ...
  test("should properly list all dependency source directories in elm.json", () => {
    expect(resolveDependencies("src/fixtures/elm-spa-example/elm.json", ELM_HOME)).toEqual([
...
describe("getIdentifiersAndEnsureCache()", () => {
  test("should respect cache", () => {
    ...
    expect(getIdentifiersAndEnsureCache([], cacheFile)).toEqual(["f1", "f2", "f3"]);
  });
  ...
  test("should generate and save cache for a dependency", () => {
    ...
    expect(getIdentifiersAndEnsureCache([`${HOME}/.elm/0.19.1/packages/elm/core/1.0.0`], cacheFile)).toEqual([
      "command",
      "init",
      "nodeList",
      "nodeListSize",
...
update`);
  ...
  test("should generate and save cache for dependencies in a project (combined with resolveDependencies())", () => {
    ...
    const deps = resolveDependencies("src/fixtures/elm-spa-example/elm.json", ELM_HOME);
    const cacheFile = path.resolve("tmp", "cache.txt");
